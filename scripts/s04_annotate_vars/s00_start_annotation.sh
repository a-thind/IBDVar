...
# create variant ids
s04_annotate_vars/s01_add_variant_ids.sh "${out_dir}" "${threads}" \
   |& tee -a "${pipeline_log}"

echo -e "------------------------------ ClinVar Annotation -----------------------------\n"
# check clinvar
s04_annotate_vars/s02_check_clinvar_vcf.sh "${out_dir}" "${clinvar}" \
    "${threads}" \
   |& tee -a "${pipeline_log}"

s04_annotate_vars/s03_rename_clinvar_chrs.sh "${out_dir}" "${clinvar}" \
    "${threads}" \
   |& tee "${pipeline_log}"

# annotate variants using clinvar
s04_annotate_vars/s04_annotate_clinvar.sh "${out_dir}" "${clinvar}" \
    "${threads}" \
    |& tee "${pipeline_log}"

s04_annotate_vars/s05_retain_std_chrs.sh "${out_dir}" "${clinvar}" \
   "${threads}" \
   |& tee "${pipeline_log}"
